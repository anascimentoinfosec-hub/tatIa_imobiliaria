...
def _renderizar_card_regra(regra_id, dados):
    ...
    with st.expander(titulo):
        ...
        with col_btn1:
            if st.button("💾 Salvar alterações", key=f"salvar_{regra_id}", use_container_width=True, type="primary"):
                _salvar_edicao(
                    regra_id=regra_id,
                    nome=novo_nome,
                    banco=novo_banco,
...
def _salvar_edicao(regra_id, nome, banco, taxa, prazo, sistema, ativo,
    ...
    """Salva as alterações de uma regra existente."""
...
def _renderizar_form_adicionar():
    ...
    with st.form("form_nova_regra"):
        ...
        with col1:
            nome = st.text_input(
                "Nome da regra",
                placeholder="Ex: Caixa - SBPE",
                help="Nome que aparecerá no simulador.",
            )
            banco = st.text_input(
                "Banco",
        ...
        with col2:
            taxa = st.number_input(
                ...
                step=0.01,
                format="%.2f",
                help="Taxa de juros anual. Ex: 11.49 significa 11,49% a.a.",
            )
            prazo = st.number_input(
                ...
                value=420,
                step=12,
                help="Prazo máximo em meses.",
            )
            entrada_min = st.number_input(
                "Entrada mínima (%)",
                min_value=0.0,
                max_value=100.0,
                value=20.0,
                step=1.0,
                help="Percentual mínimo de entrada.",
            )
        ...
        comprometimento_max = st.number_input(
            ...
            value=30.0,
            step=1.0,
            help="Percentual máximo da renda comprometida com a parcela.",
        )
